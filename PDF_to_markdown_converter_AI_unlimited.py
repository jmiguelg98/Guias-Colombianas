#!/usr/bin/env python3
# PDF to Markdown Converter - AI UNLIMITED VERSION
# Uses AI Vision for EVERY image - Maximum analysis quality
# For any type of guidelines and documents
...
class PDFToMarkdownConverterUnlimited:
    def __init__(self):
            ...
            for page_num in range(len(doc)):
                ...
                for img_index, img in enumerate(image_list):
                    try:
                        ...
                        # Process ALL images with AI (unlimited version)
                        if pix.n - pix.alpha <= 3:
                            try:
                                img_data = pix.tobytes("png")
                                img_pil = Image.open(io.BytesIO(img_data))
                                
                                # Use AI for every single image
                                image_content = self.process_image_with_ai(img_pil, page_num + 1, img_index + 1)
    ...
    def get_ai_image_description(self, img_pil: Image.Image, page_num: int, img_index: int) -> str:
        """Get AI description of image using OpenAI GPT-4o Vision"""
        try:
                ...
                "messages": [
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": "Analyze this image from a medical/clinical guideline document. Describe all visible content including: text, diagrams, flowcharts, tables, figures, charts, decision trees, algorithms, or any other visual elements. Be comprehensive and detailed. Focus on extracting actionable information and clinical relevance."
                            },
                            {
                                "type": "image_url",
                                "image_url": {
    ...
    def process_image_with_ai(self, img_pil: Image.Image, page_num: int, img_index: int) -> str:
        """Process every image with AI Vision + OCR backup"""
        try:
            self.total_images_processed += 1
            
            # Always try AI first for unlimited version
            if self.use_ai_vision:
                self.ai_calls_count += 1
                # Dynamic cost calculation (GPT-4o Vision pricing)
                cost_per_call = 0.01275  # $0.01275 per image (current GPT-4o pricing)
                self.current_cost_estimate = self.ai_calls_count * cost_per_call
                
                logger.info(f"💰 AI call #{self.ai_calls_count} (Current cost: ${self.current_cost_estimate:.2f}) - Processing image {img_index} on page {page_num}")
                
                ai_description = self.get_ai_image_description(img_pil, page_num, img_index)
                
                if ai_description:
                    # Also get OCR as backup/additional info
                    try:
                        ocr_text = pytesseract.image_to_string(img_pil, lang='eng').strip()
                        if len(ocr_text) > 5:
                        ...
                        return f"**AI Analysis:** {ai_description}"
                else:
                    # AI failed, fall back to OCR only
                    logger.warning(f"AI vision failed for image {img_index} on page {page_num}, using OCR fallback")
            
            # Fallback to OCR if AI disabled or failed
            try:
                ocr_text = pytesseract.image_to_string(img_pil, lang='eng').strip()
                if len(ocr_text) > 5:
                ...
                return "**Image detected - processing failed**"
    ...
    def extract_metadata(self, pdf_path: str) -> Dict[str, str]:
        ...
        try:
            ...
            # General entity patterns (not specific to Colombia)
            entity_patterns = [
                r'Association of [A-Za-z\s]+',
                r'Society of [A-Za-z\s]+',
                r'American [A-Za-z\s]+ Association',
                r'European [A-Za-z\s]+ Society',
                r'International [A-Za-z\s]+',
                r'College of [A-Za-z\s]+',
                r'Academy of [A-Za-z\s]+',
                r'Federation of [A-Za-z\s]+',
                r'WHO',
            ...
            if not metadata['title']:
                ...
                for line in lines[:10]:
                    ...
                    if len(line) > 20 and len(line) < 200:
                        keywords = ['guideline', 'consensus', 'recommendation', 'protocol', 'standard']
    ...
    def create_markdown_content(self, metadata: Dict[str, str], text_content: str, 
        ...
        content_parts.append(f"*Converted from PDF to Markdown for LLM processing*")
        content_parts.append(f"*AI Vision calls used: {self.ai_calls_count} (Est. cost: ${self.current_cost_estimate:.2f})*")
    ...
    def convert_pdf_to_markdown(self, pdf_path: str, output_dir: str) -> bool:
        try:
            ...
            logger.info("Extracting and processing images with AI...")
    ...
    def convert_batch(self, input_dir: str, output_dir: str, progress_callback=None) -> Dict[str, int]:
        ...
        print(f"\n🚀 Processing {results['total']} PDF files with AI Vision...")
        print("=" * 60)
        ...
        for i, pdf_path in enumerate(pdf_files):
            try:
                ...
                if self.convert_pdf_to_markdown(str(pdf_path), output_dir):
                    results['success'] += 1
                    print(f"   ✅ Success (Current AI cost: ${self.current_cost_estimate:.2f})")
                else:
                    results['failed'] += 1
                    print(f"   ❌ Failed")
...
class PDFConverterGUIUnlimited:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("PDF to Markdown Converter - AI UNLIMITED (Every Image)")
        self.root.geometry("700x550")
        
        self.converter = PDFToMarkdownConverterUnlimited()
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(main_frame, text="PDF to Markdown Converter - AI UNLIMITED", 
                               font=('Arial', 16, 'bold'))
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 20))
        
        warning_label = ttk.Label(main_frame, text="🔥 AI Vision for EVERY image + No filename conflicts!", 
        ...
        ai_frame = ttk.LabelFrame(main_frame, text="🔥 AI Vision (REQUIRED - Every Image)", padding="5")
        ...
        ttk.Label(ai_frame, text="🔥 UNLIMITED: AI analyzes every single image", font=('Arial', 9)).grid(row=1, column=0, columnspan=2, sticky=tk.W)
        ttk.Label(ai_frame, text="🔥 Maximum quality analysis for all guidelines", font=('Arial', 9)).grid(row=2, column=0, columnspan=2, sticky=tk.W)
        ttk.Label(ai_frame, text="💰 Cost: ~$0.01-0.02 per image (real-time tracking)", font=('Arial', 9), foreground='red').grid(row=3, column=0, columnspan=2, sticky=tk.W)
        
        convert_button = ttk.Button(main_frame, text="Convert PDFs (AI UNLIMITED)", command=self.start_conversion)
        convert_button.grid(row=5, column=0, columnspan=3, pady=20)
        
        self.progress_var = tk.StringVar()
        self.progress_var.set("Ready for unlimited AI processing!")
    ...
    def update_progress(self, current: int, total: int, current_file: str):
        ...
        cost = self.converter.current_cost_estimate
        status_text = f"Processing {current}/{total}: {filename} (Cost: ${cost:.2f})"
    ...
    def start_conversion(self):
        ...
        if not api_key:
            messagebox.showerror("Error", "OpenAI API key is required for unlimited AI processing")
            return
        
        self.converter.set_openai_key(api_key)
        self.log_message("🔥 AI Vision UNLIMITED enabled - every image will be analyzed!")
        
        self.log_text.delete(1.0, tk.END)
        self.log_message("🔥 AI-UNLIMITED VERSION - Maximum analysis quality!")
    ...
    def run_conversion(self):
        try:
            self.log_message("Starting PDF to Markdown conversion with unlimited AI analysis...")
            ...
            final_cost = self.converter.current_cost_estimate
            self.progress_var.set(f"Conversion completed - Total cost: ${final_cost:.2f}")
            success_count = results['success']
            failed_count = results['failed']
            completion_message = f"Conversion completed! Success: {success_count}, Failed: {failed_count}"
            self.log_message(completion_message)
            
            # Cost summary
            self.log_message(f"💰 FINAL COST: {self.converter.ai_calls_count} AI calls = ${final_cost:.2f}")
            self.log_message(f"📊 Total images processed: {self.converter.total_images_processed}")
            
            dialog_lines = [
                "🔥 AI-UNLIMITED conversion completed!",
                "",
                "📊 PROCESSING SUMMARY:",
                ...
                f"Total images processed: {self.converter.total_images_processed}",
            ...
            messagebox.showinfo("AI-Unlimited Conversion Complete!", dialog_message)
...
def main():
    ...
    print("PDF to Markdown Converter - AI UNLIMITED VERSION")
    print("=" * 55)
    print("🔥 AI Vision for EVERY image - Maximum analysis quality")
    print("🔧 For any type of guidelines and documents")
    print("💰 Real-time cost tracking with dynamic estimates")
    print("=" * 55)
