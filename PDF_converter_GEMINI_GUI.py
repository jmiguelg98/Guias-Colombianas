#!/usr/bin/env python3
"""
PDF to Markdown Converter - GEMINI VISION GUI VERSION
Uses Google Gemini's advanced vision capabilities for medical image analysis
...
try:
    ...
    print("Please install: pip install PyMuPDF Pillow pytesseract google-generativeai")
    sys.exit(1)
...
class GeminiCostTracker:
    ...
    def __init__(self):
        ...
        # Gemini 1.5 Pro pricing (current rates)
        self.image_analysis_cost = 0.00265  # $2.65 per 1K images = $0.00265 per image
        self.text_analysis_cost = 0.00125   # $1.25 per 1M input tokens (estimated per call)
...
class GeminiVisionAnalyzer:
    """Gemini-powered medical image analysis"""
    ...
    def __init__(self, api_key: str, progress_callback=None):
        ...
        self.log_message("🔮 Gemini Vision Analyzer initialized")
    ...
    def call_gemini_vision(self, img_pil: Image.Image, prompt: str) -> str:
        ...
        try:
            ...
            self.log_message(f"💰 Gemini analysis cost: ${cost:.4f} (Total: ${self.cost_tracker.total_cost:.3f})")
    ...
    def classify_image_content(self, img_pil: Image.Image) -> str:
        """Classify medical image content using Gemini"""
...
Respond with just the classification: TABLE, FLOWCHART, FIGURE, TEXT, or MIXED"""
        ...
        response = self.call_gemini_vision(img_pil, prompt)
    ...
    def analyze_table_content(self, img_pil: Image.Image, page_num: int, img_index: int) -> str:
        """Comprehensive table analysis with Gemini"""
...
FORMAT: Provide a structured analysis that preserves all data while explaining clinical significance."""
        ...
        self.log_message(f"📊 Analyzing medical table {img_index} (Page {page_num}) with Gemini")
        
        analysis = self.call_gemini_vision(img_pil, prompt)
        self.tables_found += 1
    ...
    def analyze_flowchart_content(self, img_pil: Image.Image, page_num: int, img_index: int) -> str:
        """Comprehensive flowchart analysis with Gemini"""
...
Make this analysis practical for healthcare providers to implement."""
        ...
        self.log_message(f"🌳 Analyzing medical flowchart {img_index} (Page {page_num}) with Gemini")
    ...
    def analyze_figure_content(self, img_pil: Image.Image, page_num: int, img_index: int) -> str:
        """Comprehensive figure analysis with Gemini"""
...
Provide a detailed description that would help someone understand the medical content without seeing the image."""
        ...
        self.log_message(f"🔬 Analyzing medical figure {img_index} (Page {page_num}) with Gemini")
    ...
    def analyze_text_content(self, img_pil: Image.Image, page_num: int, img_index: int) -> str:
        """Comprehensive text analysis with Gemini"""
...
Provide both the complete text and medical analysis."""
        ...
        self.log_message(f"📝 Analyzing medical text {img_index} (Page {page_num}) with Gemini")
    ...
    def analyze_mixed_content(self, img_pil: Image.Image, page_num: int, img_index: int) -> str:
        """Comprehensive mixed content analysis with Gemini"""
...
Provide a unified analysis that captures all content types and their relationships."""
        ...
        self.log_message(f"🔄 Analyzing mixed medical content {img_index} (Page {page_num}) with Gemini")
    ...
    def process_image(self, img_pil: Image.Image, page_num: int, img_index: int) -> str:
        """Process image with Gemini's comprehensive analysis"""
        try:
            # First classify with Gemini
...
class GeminiPDFAnalyzer:
    """Main PDF analyzer using Gemini Vision"""
    
    def __init__(self, api_key: str, progress_callback=None):
        self.api_key = api_key
        self.progress_callback = progress_callback
        self.vision_analyzer = GeminiVisionAnalyzer(api_key, progress_callback)
    ...
    def extract_and_analyze_images(self, pdf_path: str) -> List[str]:
        """Extract and analyze images with Gemini"""
        image_analyses = []
        ...
        try:
            ...
            self.log_message("🔮 Starting Gemini image analysis...")
            ...
            self.log_message("🎉 GEMINI ANALYSIS COMPLETE!")
    ...
    def convert_pdf_with_gemini(self, pdf_path: str, output_dir: str) -> bool:
        """Convert PDF using Gemini Vision"""
        try:
            self.log_message(f"🔮 Starting Gemini analysis of {Path(pdf_path).name}")
            ...
            self.log_message("🔍 Analyzing images with Gemini Vision...")
            ...
            new_filename = f"{clean_name}_GEMINI_{timestamp}"
    ...
    def create_markdown_output(self, text_content: str, image_analyses: List[str], 
        ...
        content_parts.append("*Analyzed with Google Gemini Vision AI*")
        content_parts.append("")
        ...
        content_parts.append(f"**Analysis Method:** Google Gemini 1.5 Pro Vision")
        content_parts.append("")
        content_parts.append("---")
        content_parts.append("")
        ...
        # Image analyses
        if image_analyses:
            content_parts.append("## 🔮 Gemini Vision Analysis Results")
            content_parts.append("")
            content_parts.append("*Each image comprehensively analyzed using Google Gemini's advanced vision capabilities*")
            content_parts.append("")
        ...
        # Summary
        summary = self.vision_analyzer.get_analysis_summary()
        content_parts.append("## 🔮 Gemini Processing Summary")
        content_parts.append("")
        content_parts.append(f"**Analysis Method:** Google Gemini 1.5 Pro Vision")
        ...
        content_parts.append(f"**Average Cost per Image:** ${summary['avg_per_image']:.4f}")
        content_parts.append("")
        content_parts.append("**Gemini Capabilities:**")
        ...
        content_parts.append("- Multi-modal reasoning")
        
        return "\n".join(content_parts)

# GUI for Gemini Version
class GeminiPDFAnalyzerGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🔮 Gemini Vision PDF Analyzer - Medical AI Analysis")
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="🔮 Gemini Vision PDF Analyzer", 
                               font=('Arial', 16, 'bold'))
        title_label.grid(row=0, column=0, columnspan=3, pady=(0, 10))
        
        subtitle_label = ttk.Label(main_frame, text="Advanced Medical Image Analysis with Google Gemini", 
                                  font=('Arial', 12), foreground='blue')
        subtitle_label.grid(row=1, column=0, columnspan=3, pady=(0, 15))
        
        # API Key
        ttk.Label(main_frame, text="Google AI Studio API Key:").grid(row=2, column=0, sticky=tk.W, pady=5)
        ...
        # Gemini info
        info_frame = ttk.LabelFrame(main_frame, text="🔮 Gemini Vision Capabilities", padding="5")
        ...
        ttk.Label(info_frame, text="💰 Cost tracking: ~$0.00265 per image analysis", font=('Arial', 9)).grid(row=4, column=0, sticky=tk.W)
        ttk.Label(info_frame, text="🆓 Get API key free at: ai.google.dev", font=('Arial', 9), foreground='green').grid(row=5, column=0, sticky=tk.W)
        
        # Convert button
        convert_button = ttk.Button(main_frame, text="Start Gemini Analysis", command=self.start_conversion)
        convert_button.grid(row=6, column=0, columnspan=3, pady=20)
        
        # Progress
        self.progress_var = tk.StringVar()
        self.progress_var.set("Ready for Gemini analysis")
        ...
        # Log
        log_frame = ttk.LabelFrame(main_frame, text="Gemini Analysis Log", padding="5")
    ...
    def start_conversion(self):
        if not self.api_key.get():
            messagebox.showerror("Error", "Please enter your Google AI Studio API key\nGet one free at: https://ai.google.dev")
            return
        ...
        self.log_message("🔮 Starting Gemini Vision analysis...")
    ...
    def run_conversion(self):
        try:
            self.analyzer = GeminiPDFAnalyzer(self.api_key.get(), self.log_message)
            ...
            for i, pdf_path in enumerate(pdf_files):
                ...
                if self.analyzer.convert_pdf_with_gemini(str(pdf_path), self.output_dir.get()):
                    success_count += 1
                
                self.root.update()
            
            self.progress_var.set("Gemini analysis completed!")
...
def main():
    print("🔮 GEMINI VISION PDF ANALYZER")
    print("=" * 50)
    print("🔬 Advanced medical image analysis")
    print("📊 Comprehensive table interpretation")
    print("🌳 Detailed flowchart analysis")
    print("💰 Affordable pricing (~$2.65/1K images)")
    print("🆓 Free API key at: https://ai.google.dev")
    print("=" * 50)
    
    app = GeminiPDFAnalyzerGUI()
    app.run()
